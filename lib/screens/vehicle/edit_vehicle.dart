    ...
    return BlocListener<AddVehicleBloc, AddVehicleState>(
      ...
      child: BlocListener<VehicleBloc, VehicleState>(
        ...
        child: BlocBuilder<AddVehicleBloc, AddVehicleState>(builder: (context, state) {
          return BlocBuilder<VehicleBloc, VehicleState>(builder: (context, editState) {
            if (editState is VehicleLoading) {
              return const Center(child: CircularProgressIndicator());
            }
            if (editState is VehicleError) {
              return Center(child: Text(editState.message));
            }
            if (editState is VehicleLoaded) {
              return Scaffold(
                body: BackgroundImage(
                  child: Column(
                    ...
                    children: [
                      SafeArea(
                          bottom: false, child: AppHeaders().extendedHeader(text: AppConstants.editProfile, context: context)),
                      verticalSpacer(
                        height: 10.0,
                      ),
                      Padding(
